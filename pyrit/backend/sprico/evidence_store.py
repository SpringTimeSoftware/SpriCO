...
class SpriCOEvidenceStore:
    ...
    def append_event(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        payload = {
            "finding_id": event.get("finding_id") or f"evidence_{uuid.uuid4().hex[:12]}",
        ...
        if self._backend is not None:
            stored = self._backend.upsert_record("evidence_items", payload["finding_id"], payload)
            if str(payload.get("engine") or "").startswith("sprico.shield"):
                self._backend.upsert_record("shield_events", payload["finding_id"], payload)
            return stored
