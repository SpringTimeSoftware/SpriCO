...
describe('GarakScannerPage', () => {
  ...
  it('explains completed scans with no actionable findings and summarizes artifacts', async () => {
    ...
    expect((await screen.findAllByText(/Completed - no findings/)).length).toBeGreaterThan(0)
    expect(screen.getByText(/This does not prove the target is safe against all attacks/)).toBeInTheDocument()
    expect(screen.getByText('PASS for selected scan scope')).toBeInTheDocument()
    expect(screen.getByText(/No actionable findings were created for the selected profile and categories/)).toBeInTheDocument()
    expect(screen.getByText('No actionable scanner evidence was produced. No Evidence Center records were created for this scan.')).toBeInTheDocument()
